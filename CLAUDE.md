...
```kotlin
val ciVersionCode = (System.getenv("VERSION_CODE") ?: "50").toInt()
```
CI sets `VERSION_CODE` from the GitHub run number + 100. The fallback (currently 50) must be incremented with each release to avoid Play Store "version code already used" errors.
